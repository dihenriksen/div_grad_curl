...
% unit vector, bold hat
\newcommand{\bh}[1]{\hat{\bb{#1}}}
\maketitle
...
\begin{enumerate}
    ...
    (c) $\bb{F} = (\vvec{x}{y}{z}) e^{-(x^2 + y^2 + z^2)}$. S, the surface of the sphere of radius R centered at the origin as shown in the figure.

    Solution:

    Similar to the previous problem but in spherical coordinates, the normal vector to the sphere's surface is:
    \[ \bh{n} = \bh{r} = \frac{(x\bh{i} + y\bh{j} + z\bh{k})}{r} \]
    \[\bb{F} \cdot \bh{n} = \frac{x^2 + y^2 + z^2}{r}e^{-(x^2 + y^2 + z^2)} = re^{-(r^2)} \]
    For this spherical shell:
    \[ r = R, \quad \theta \in (0,2\pi), \quad \phi \in (0,\pi) \]
    The dot product will be a constant again, and the rest of the integral gives the surface area of a sphere of radius R:
    \[ \iint_S \bb{F} \cdot \hat{\bb{n}} dS = \int_0^{2\pi} \int_0^\pi re^{-(r^2)} r^2 \sin\theta d\phi d\theta = R^3 e^{-(R^2)} \int_0^{2\pi} \int_0^\pi \sin\theta d\phi d\theta\]
    \[ = 4 \pi R^3 e^{-(R^2)} \]

    (d) $\bb{F} = \bb{i}E(x)$ where E(x) is an arbitrary scalar function of x. S, the surface of the cube of side b shown in the figure
    (bottom corner at the origin, and the entire cube in the first octant).

    Solution:

    Faces with a unit normal vector of $\bh{j}$ or $\bh{k}$ will have yield a dot product with \bb{F} of zero. The nonzero faces are the two parallel
    to the yz-plane, one of which has a +\bb{i} normal vector and the other has a -\bb{i}. Thus the surface integral is broken into two smaller surface integrals:
    \[ \iint_S \bb{F} \cdot \hat{\bb{n}} dS = \iint_S E(x) dydz \bigg{|}_{x=b} + \iint_S -E(x) dydz \bigg{|}_{x=0} \]
    \[ = \iint_S E(b) dydz + \iint_S -E(0) dydz \]
    The double integral is the area of the face of the cube, which is $b^2$.
    \[ = E(b)b^2 - E(0)b^2 \]
    \[ = (E(b) - E(0))b^2 \]

    \item (a) Use Gauss' law and symmetry to find the electrostatic field as a function of position for an
    infinite uniform plane of charge. Let the charge lie in the yz-plane and denote the charge
    per unit area by $\sigma$.

    Solution:

    \[ \iint_S \bb{E} \cdot \bb{n} dS = \frac{q}{\epsilon_0} \]
    ...
    All the same symmetry considerations abide as in part (a). There are two regions that matter, inside and outside the
    cylinder of charge. For $r < b$, the enclosed charge is:
    \[ q = \rho_0 * volume = \rho_0 \pi r^2 L\]
    \[ \iint_S \bb{E(r)} dS = \frac{\rho_0 \pi r^2 L}{\epsilon_0} \]
    \[ E(r) 2 \pi rL = \frac{\rho_0 \pi r^2 L}{\epsilon_0} \]
    \[ E(r) = \frac{\rho_0 r}{2\epsilon_0} \]
    \[ E(r) = \frac{\rho_0 r}{2\epsilon_0} \bh{e}_r, \; r < b \]

    For $r > b$, the enclosed charge is:
    \[ q = \rho_0 \pi b^2 L \]
    \[ \iint_S \bb{E(r)} dS = \frac{\rho_0 \pi b^2 L}{\epsilon_0} \]
    \[ E(r) 2 \pi rL = \frac{\rho_0 \pi b^2 L}{\epsilon_0} \]
    \[ E(r) = \frac{\rho_0 b^2}{2r\epsilon_0} \]
    \[ E(r) = \frac{\rho_0 b^2}{2r\epsilon_0} \bh{e}_r, \; r > b \]
    ...
    \[ \iint_S \bb{E(r)} dS = \frac{q}{\epsilon_0} \]
    \[ q = \iiint_V \rho(r) dV = \iiint_V \rho_0 e^{(-r/b)} dV \]
    The volume in this case is a cylinder, so we can use cylindrical coordinates:
    \[ q = \rho_0 \int_0^L \int_0^{2\pi} \int_0^r e^{(-x/b)} xdx d\phi dz \]
    \[ q = \rho_0 2 \pi L \int_0^r e^{(-x/b)} xdx \]
    \centerline{TODO evaluate through integration by parts}
    \[ q = \rho_0 2 \pi L b \left[ b - e^{-r/b}(b + r) \right] \]
    \[ q = \rho_0 2 \pi L b^2 \left[ 1 - e^{-r/b}(1 + r/b) \right] \]
    \[ E(r) \iint_S dS = \frac{\rho_0 2 \pi L b^2 \left[ 1 - (1 + r/b)e^{-r/b} \right]}{\epsilon_0} \]
    \[ E(r) (2 \pi r L) = \frac{\rho_0 2 \pi L b^2 \left[ 1 - (1 + r/b)e^{-r/b} \right]}{\epsilon_0} \]
    \[ E(r) = \frac{\rho_0}{\epsilon_0} \frac{b^2}{r} \left[ 1 - (1 + r/b)e^{-r/b} \right] \bh{e}_r\]

    \item (a) Use Gauss' law and symmetry to find the electrostatic field as a function of position
    for the spherically symmetric charge distribution whose density is given in spherical coordinates
    by
    \begin{equation*}
        \rho (r) =
            \begin{cases}
